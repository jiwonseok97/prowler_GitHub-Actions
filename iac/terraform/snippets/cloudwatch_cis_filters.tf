...
locals {
  cw_enabled = var.cloudwatch_log_group_name != ""
}

# ── Shared: SNS topic for all CIS alarms ──
resource "aws_sns_topic" "cis_alarms" {
  count = local.cw_enabled ? 1 : 0
  name  = "cis-cloudwatch-alarms"
}

resource "aws_sns_topic_policy" "cis_alarms" {
  count = local.cw_enabled ? 1 : 0
  arn   = aws_sns_topic.cis_alarms[0].arn
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      ...
      Action    = "SNS:Publish"
      Resource  = aws_sns_topic.cis_alarms[0].arn
    }]
  })
}
...
resource "aws_cloudwatch_metric_alarm" "unauthorized_api_calls" {
  ...
  alarm_actions       = [aws_sns_topic.cis_alarms[0].arn]
}
...
resource "aws_cloudwatch_log_metric_filter" "kms_cmk_deletion" {
  count          = local.cw_enabled ? 1 : 0
...
resource "aws_cloudwatch_metric_alarm" "kms_cmk_deletion" {
  count               = local.cw_enabled ? 1 : 0
