...
3. Creates a new `aws_s3_bucket_policy` resource to enforce HTTPS-only access to the S3 bucket.
   - The policy denies all actions (`s3:*`) on the bucket and its objects when the `aws:SecureTransport` condition is `false`, effectively enforcing HTTPS-only access.