...
# Attach a bucket policy to the log target bucket to allow the source bucket to write logs
...
# Define input variables for the S3 bucket name and IAM role/policy ARNs
...
variable "iam_role_arn" {
  description = "ARN of the IAM role to be used by CloudTrail"
  type        = string
  default     = ""
}

variable "iam_policy_arn" {
  description = "ARN of the IAM policy to be attached to the CloudTrail IAM role"
  type        = string
  default     = ""
}

# Attach the required IAM policy to the CloudTrail IAM role
