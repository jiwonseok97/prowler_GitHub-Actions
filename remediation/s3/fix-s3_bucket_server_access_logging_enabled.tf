...
# Create a CloudWatch alarm to monitor S3 bucket access logging
resource "aws_cloudwatch_metric_alarm" "remediation_s3_bucket_logging_enabled" {
  alarm_name          = "remediation-s3-bucket-logging-enabled"
  ...
  statistic           = "Sum"
  threshold           = 0
  alarm_description   = "Alarm when S3 bucket logging is not enabled"
