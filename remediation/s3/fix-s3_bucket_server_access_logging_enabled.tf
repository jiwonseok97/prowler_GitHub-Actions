...
# Attach the CloudTrail log delivery policy to the target S3 bucket
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Action   = "s3:PutObject",
        Resource = "${aws_s3_bucket.remediation_cloudtrail_logs_target.arn}/AWSLogs/${data.aws_caller_identity.current.account_id}/*"
      },
      {
        Effect = "Allow",
        Principal = {
...
# Enable CloudTrail logging for the S3 bucket
