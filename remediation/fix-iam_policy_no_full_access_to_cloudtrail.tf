...
resource "aws_iam_policy" "cloudtrail_policy" {
  ...
  description = "Allows access to required CloudTrail actions"
  policy      = data.aws_iam_policy_document.cloudtrail_policy_document.json
}

# Attach the new CloudTrail policy to the existing IAM policy
...
5. Attaches the new CloudTrail policy to the existing IAM policy.

This should address the security finding by applying the principle of least privilege and allowing only the required CloudTrail actions.