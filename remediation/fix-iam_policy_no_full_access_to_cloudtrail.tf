...
resource "aws_iam_policy" "aws_learner_dynamodb_policy" {
  name        = "aws_learner_dynamodb_policy"
  description = "IAM policy for DynamoDB access with limited CloudTrail permissions"
...
3. Creates a new IAM policy document with the required CloudTrail permissions (GetTrailStatus, DescribeTrails, LookupEvents).
4. Creates a new IAM policy with the updated permissions, using the policy document from the previous step.

The new policy will replace the existing `aws_learner_dynamodb_policy` and grant the necessary CloudTrail permissions while avoiding the `cloudtrail:*` privilege.