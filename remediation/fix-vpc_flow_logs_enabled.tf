...
3. Creates a VPC flow log to capture all traffic (both `ACCEPT` and `REJECT`) and sends the logs to a CloudWatch Logs log group.
4. Creates an IAM role for the VPC flow log and attaches the `AWSVPCFlowLogsFullAccess` policy to it, applying the principle of least privilege.