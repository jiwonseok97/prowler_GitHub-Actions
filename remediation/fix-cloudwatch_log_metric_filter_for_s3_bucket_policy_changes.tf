...
2. Creates a CloudWatch log metric filter that captures S3 bucket policy changes. The filter looks for events with the `eventSource` of `s3.amazonaws.com` and the `eventName` of `PutBucketPolicy`.
3. Creates a CloudWatch metric alarm that monitors the S3 bucket policy changes metric filter. The alarm is triggered when the metric value is greater than or equal to 1, indicating that a bucket policy change has been detected. The alarm action is set to send a notification to an SNS topic named `my-security-topic`.