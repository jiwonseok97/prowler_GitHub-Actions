...
4. Applies the `private` ACL to the S3 bucket using the `aws_s3_bucket_acl` resource, which ensures that the bucket owner has full control.

The `depends_on` attribute in the `aws_s3_bucket_acl` resource ensures that the `aws_s3_bucket_ownership_controls` resource is applied before the `aws_s3_bucket_acl` resource, as the bucket owner enforcement must be in place before setting the ACL.