...
2. Creates a CloudWatch Logs metric filter named `ConsoleLoginFailures` that tracks failed console login attempts with the error message "Failed authentication".
3. Creates a CloudWatch alarm named `ConsoleLoginFailuresAlarm` that triggers when the `AuthenticationFailures` metric (defined in the metric filter) is greater than or equal to 5 within a 1-minute period.
4. The alarm action is set to an SNS topic `arn:aws:sns:ap-northeast-2:132410971304:security-alerts`, which can be used to send notifications about the authentication failures.