...
resource "aws_cloudwatch_metric_alarm" "console_login_failures_alarm" {
  ...
  statistic           = "Sum"
  threshold           = "5"
  alarm_description   = "Alarm for AWS Management Console authentication failures"
...
3. Creates a CloudWatch alarm for the `ConsoleLoginFailures` metric. The alarm is triggered when the sum of the metric values is greater than or equal to 5 within a 1-minute period. The alarm action is to send a notification to the `arn:aws:sns:ap-northeast-2:132410971304:security-alerts` SNS topic.