...
2. Creates a CloudWatch Logs metric filter for `ConsoleLogin` failures, where the error message is "Failed authentication". The metric is named "AuthenticationFailures" and is recorded in the "SecurityMetrics" namespace.
3. Creates a CloudWatch alarm for the "AuthenticationFailures" metric, which triggers when the sum of failures in a 1-minute period is greater than or equal to 5. The alarm action is to send a notification to the "arn:aws:sns:ap-northeast-2:132410971304:security-alerts" SNS topic.