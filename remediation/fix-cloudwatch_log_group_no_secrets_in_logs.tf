...
3. Applies a data protection policy to the CloudWatch log group using the `aws_cloudwatch_log_group_policy` resource.
   - The policy allows certain actions (Describe, Get, List, StartQuery, StopQuery, TestMetricFilter, FilterLogEvents) for all principals on the log group.
   - The policy denies the `logs:Unmask` action for all principals on the log group, which helps prevent the exposure of sensitive data in the logs.