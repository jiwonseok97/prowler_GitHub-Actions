...
3. Creates a CloudWatch alarm that triggers when the "VPCRouteTableChanges" metric, as defined in the metric filter, is greater than or equal to 1. This will alert the responders when a VPC route table change is detected.
4. The alarm action is set to an SNS topic, which can be used to notify the appropriate security team.