...
2. Creates a CloudWatch log metric filter that captures any `ConsoleLogin` events where `MFAUsed` is not "Yes". This will track instances of users signing in to the Management Console without MFA.
3. Creates a CloudWatch alarm that monitors the `SignInWithoutMFA` metric and triggers an alarm when the value is greater than or equal to 1, indicating a sign-in without MFA has occurred. The alarm is configured to send notifications to the `my-security-alerts` SNS topic.