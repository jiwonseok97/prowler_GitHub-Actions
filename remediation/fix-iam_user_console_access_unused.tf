...
# Use the data source to get the existing IAM user
...
resource "aws_iam_user_login_profile" "prowler" {
  user    = data.aws_iam_user.prowler.user_name
...
2. Uses the `data` source to get the existing IAM user named `prowler`.
