...
resource "aws_network_interface_sg_attachment" "remediation_sg_attachment" {
  ...
  network_interface_id = data.aws_instances.existing_instances.ids[count.index]
}

# Optionally, you can create a new launch template that uses the remediation security group
resource "aws_launch_template" "remediation_launch_template" {
  ...
  # Add other required configuration for the launch template
}
