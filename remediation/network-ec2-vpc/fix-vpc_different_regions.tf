...
resource "aws_route_table_association" "remediation_public_subnet_association" {
...
resource "aws_nat_gateway" "remediation_nat_gateway" {
  allocation_id = aws_eip.remediation_eip.id
  subnet_id     = aws_subnet.remediation_public_subnet.id
  tags = {
...
resource "aws_eip" "remediation_eip" {
  tags = {
    Name = "Remediation EIP"
  }
}
...
resource "aws_route_table_association" "remediation_private_subnet_association" {
