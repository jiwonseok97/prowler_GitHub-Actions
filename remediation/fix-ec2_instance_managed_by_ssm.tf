...
6. Attaches the necessary permissions to the IAM role using the `aws_iam_role_policy_attachment` resource.
7. Associates the IAM role with the EC2 instance using the `aws_iam_instance_profile` and `aws_ec2_instance_profile` resources.