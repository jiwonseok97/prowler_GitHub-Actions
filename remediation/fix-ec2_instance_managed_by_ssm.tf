...
resource "aws_ssm_activation" "remediation_ssm_activation" {
  ...
  description        = "Enroll EC2 instance as a Systems Manager managed node"
...
resource "aws_instance" "remediation_ec2_instance" {
  ...
  iam_instance_profile = data.aws_iam_instance_profile.current.name

  vpc_security_group_ids = tolist(data.aws_instance.current.vpc_security_group_ids)
