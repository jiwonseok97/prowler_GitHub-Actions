...
resource "aws_ssm_association" "instance_ssm_association" {
  ...
  parameters = {
    "commands" = ["echo 'Instance is now managed by AWS Systems Manager'"]
  }
}
...
5. Creates a least privilege IAM role and attaches the `AmazonSSMManagedInstanceCore` policy to it using the `aws_iam_role` and `aws_iam_role_policy_attachment` resources.
6. Attaches the least privilege IAM role to the EC2 instance using the `aws_iam_instance_profile` and `aws_instance` resources.