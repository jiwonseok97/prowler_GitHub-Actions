...
2. Retrieves the existing AWS Organization using the `data` source.
3. Ensures the AWS account is a member of the AWS Organization using the `aws_organizations_account` resource.
4. Creates an Organizational Unit (OU) for better structure and management using the `aws_organizations_organizational_unit` resource.
5. Applies a Service Control Policy (SCP) to the OU for enforcing least privilege access using the `aws_organizations_policy` and `aws_organizations_policy_attachment` resources.