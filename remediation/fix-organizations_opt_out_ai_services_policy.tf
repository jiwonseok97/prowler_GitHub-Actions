...
resource "aws_organizations_policy" "ai_services_opt_out" {
  ...
  description = "Opt out of all AI services and prohibit child accounts from overriding the policy"
...
3. Attaches the "AI Services Opt-Out" policy to the root of the AWS Organization, ensuring that the policy applies to all child accounts and cannot be overridden.

This configuration addresses the security finding by establishing an organization-wide AI services opt-out policy, aligning with the principles of least privilege and data minimization.