...
resource "aws_security_group" "web_alb_sg" {
  ...
  ingress {
    ...
    to_port     = 80
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"] # Allow inbound HTTP traffic from the internet
  }
  ...
  egress {
    ...
    to_port         = 0
    protocol        = "-1"
    security_groups = [aws_security_group.restricted_sg.id] # Allow outbound traffic to the EC2 instance
  }
}
...
# Create a new target group for the EC2 instance
...
1. Creating a new security group (`restricted_sg`) that allows only necessary inbound traffic, such as SSH access from a bastion host or via Session Manager, and application-specific traffic (e.g., HTTP/HTTPS) via the Application Load Balancer.
2. Creating a new Application Load Balancer security group (`web_alb_sg`) that allows inbound HTTP traffic from