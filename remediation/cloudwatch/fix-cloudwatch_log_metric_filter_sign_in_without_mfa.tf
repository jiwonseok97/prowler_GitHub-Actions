...
resource "aws_sns_topic_policy" "remediation_cis_alarms" {
  count = local.cw_enabled ? 1 : 0
  arn   = aws_sns_topic.remediation_cis_alarms[0].arn
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      ...
      Action    = "SNS:Publish"
      Resource  = aws_sns_topic.remediation_cis_alarms[0].arn
    }]
  })
}
...
resource "aws_cloudwatch_metric_alarm" "remediation_unauthorized_api_calls" {
  ...
  alarm_actions       = [aws_sns_topic.remediation_cis_alarms[0].arn]
}
