...
2. Creates a CloudWatch Logs metric filter for AWS Organizations changes, using the log group with the provided ARN.
3. Creates a CloudWatch alarm that triggers when there are changes to AWS Organizations, based on the metric filter created in the previous step. The alarm action is set to an SNS topic with the ARN `arn:aws:sns:ap-northeast-2:132410971304:my-alert-topic`.