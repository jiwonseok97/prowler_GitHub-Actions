...
5. Grants the required IAM permissions to the KMS key, allowing the `root` user to perform the necessary actions (encrypt, decrypt, re-encrypt, generate data key, and describe the key).