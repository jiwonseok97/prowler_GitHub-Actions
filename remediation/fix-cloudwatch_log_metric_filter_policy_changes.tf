...
resource "aws_cloudwatch_metric_alarm" "remediation_iam_policy_changes_alarm" {
  ...
  alarm_actions       = [data.aws_sns_topic.remediation_iam_policy_changes_topic.arn]
}

# Use a data source to reference an existing SNS topic for the alarm
data "aws_sns_topic" "remediation_iam_policy_changes_topic" {
  name = "remediation-iam-policy-changes-topic"
}