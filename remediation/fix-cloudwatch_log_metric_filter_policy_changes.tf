...
3. Creates a CloudWatch alarm that triggers when there is at least one IAM policy change event, and sends an alert to the `arn:aws:sns:ap-northeast-2:132410971304:security-alerts` SNS topic.

This code addresses the security finding by creating the necessary metric filter and alarm to monitor and alert on IAM policy changes, as recommended in the finding.