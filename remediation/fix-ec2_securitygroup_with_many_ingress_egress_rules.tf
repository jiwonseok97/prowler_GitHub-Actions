...
3. Creates a new security group with a reduced number of rules, following the recommended best practices:
...
4. Applies the new security group to the resources that use the existing one, using the `lifecycle` block to trigger a replacement when the existing security group changes.