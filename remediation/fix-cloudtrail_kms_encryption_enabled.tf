...
resource "aws_cloudtrail_service_account" "current" {
  account_id = data.aws_cloudtrail_service_account.current.id
}

resource "aws_cloudtrail" "security_cloudtrail" {
  name                          = data.aws_cloudtrail_trail.security_cloudtrail.name
...
2. Retrieves the existing CloudTrail trail using the `aws_cloudtrail_trail` data source.
