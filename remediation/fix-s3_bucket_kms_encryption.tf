...
2. Uses a data source to reference the existing S3 bucket `aws-cloudtrail-logs-132410971304-0971c04b`.
3. Enables default server-side encryption with a customer-managed KMS key for the S3 bucket.
4. Creates a customer-managed KMS key for the S3 bucket, with key rotation enabled.
