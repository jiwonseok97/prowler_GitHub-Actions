...
2. Retrieves the existing IAM user `github-actions-prowler` using the `data.aws_iam_user` data source.
...
4. Attaches the new IAM policy to the existing IAM user `github-actions-prowler` using the `aws_iam_user_policy_attachment` resource.