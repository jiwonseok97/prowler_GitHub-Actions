...
# Terminate the outdated instance and launch a new one using the updated template
...
5. Creates a new Auto Scaling group with the updated launch template using the `aws_autoscaling_group` resource.
6. Configures the Auto Scaling group to perform a rolling instance refresh, replacing the outdated instance with a new one using the latest AMI.