...
2. Creates a CloudWatch Logs metric filter named `RootAccountUsage` that captures root account usage events.
3. Creates a CloudWatch alarm named `RootAccountUsageAlarm` that triggers when the `RootUsage` metric is greater than or equal to 1, indicating root account usage. The alarm is configured to send notifications to the `my-alert-topic` SNS topic.