...
# Create a CloudWatch log group subscription filter to restrict cross-account access
...
3. Creates a CloudWatch log subscription filter to restrict cross-account access to the log group. The `role_arn` parameter is set to the specified role ARN, and the `destination_arn` parameter is set to a custom log destination.