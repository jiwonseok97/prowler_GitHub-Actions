...
3. Creates a CloudWatch log subscription filter to restrict cross-account sharing. The `role_arn` parameter is set to the existing role ARN, and the `destination_arn` parameter is set to a specific destination ARN.