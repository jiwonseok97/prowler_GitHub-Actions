...
3. Creates a CloudWatch alarm that triggers when the metric filter detects one or more configuration changes.
4. The alarm action is set to an SNS topic, which can be used to notify responders of the configuration changes.