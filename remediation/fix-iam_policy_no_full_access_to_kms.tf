...
3. Creates a new IAM policy document with the recommended changes:
   - Allows only the necessary KMS actions (`Encrypt`, `Decrypt`, `ReEncrypt*`, `GenerateDataKey*`, `DescribeKey`) scoped to the specific key ARN.
   - Adds a condition to restrict the KMS access to only the `dynamodb.amazonaws.com` service.
4. Creates a new IAM policy using the updated policy document.