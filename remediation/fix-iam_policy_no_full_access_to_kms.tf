...
# Create the updated IAM policy using the new policy document
...
3. Creates a new IAM policy document with the recommended changes:
   - Allows only the necessary KMS actions: `Encrypt`, `Decrypt`, `ReEncrypt*`, `GenerateDataKey*`, and `DescribeKey`.
   - Scopes the KMS key access to the specific key ARN `arn:aws:kms:ap-northeast-2:132410971304:key/*`.
   - Adds a condition to only allow access via the `dynamodb.amazonaws.com` service.
