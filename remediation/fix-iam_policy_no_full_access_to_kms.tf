...
# Attach the new IAM policy document to the existing IAM policy
...
3. Creates a new IAM policy document with the recommended changes:
   - Allows only the necessary KMS actions (`Encrypt`, `Decrypt`, `ReEncrypt*`, `GenerateDataKey*`, `DescribeKey`) scoped to the specific key ARNs.
   - Adds a condition to restrict the KMS access to only the `dynamodb.amazonaws.com` service.
4. Attaches the new IAM policy document to the existing IAM policy using the `aws_iam_policy` resource.