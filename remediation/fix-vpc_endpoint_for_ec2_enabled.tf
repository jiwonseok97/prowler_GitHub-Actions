...
3. Creates a new VPC endpoint for the EC2 service using the `aws_vpc_endpoint` resource.
   ...
   - A restrictive endpoint policy is applied to the VPC endpoint, allowing all EC2 actions but restricting access to the specific VPC.

This Terraform code should help address the security finding by creating an interface VPC endpoint for the EC2 service, enabling private DNS, and applying a restrictive endpoint policy.